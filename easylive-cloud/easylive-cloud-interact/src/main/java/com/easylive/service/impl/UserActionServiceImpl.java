...
public class UserActionServiceImpl implements UserActionService {
	...
	public void saveAction(UserAction bean) {
		VideoInfo videoInfo = (VideoInfo) videoClient.getVideoInfoByVideoId(bean.getVideoId());
		if (videoInfo == null) {
		...
		switch (actionTypeEnum) {
			...
			case VIDEO_COLLECT://两个case合并处理 因为逻辑相似
				...
				videoClient.updateCountInfo(bean.getVideoId(), actionTypeEnum.getField(), changeCount);//借用之前在xml实现的弹幕加一功能实现点赞的增加
