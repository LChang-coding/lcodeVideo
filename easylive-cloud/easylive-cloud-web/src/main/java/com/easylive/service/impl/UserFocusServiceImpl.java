...
public class UserFocusServiceImpl implements UserFocusService {
	...
	public void focusUser(String userId, String focusUserId) {
		if(userId.equals(focusUserId))throw new BusinessException("不能关注自己");
		UserFocus dbInfo= (UserFocus) userFocusMapper.selectByUserIdAndFocusUserId(userId,focusUserId);//查数据库 是否关注完成
		if(dbInfo!=null)return;
		UserInfo userInfo= (UserInfo) userInfoMapper.selectByUserId(userId);//查询用户是否存在
	...
	public Integer addBatch(List<UserFocus> listBean) {
        ...
        if (listBean != null && !listBean.isEmpty()) {
            result = this.userFocusMapper.insertBatch(listBean);
        }
        return result;
    }
